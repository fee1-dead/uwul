...
use uwuc_base::lex::{ErrorKind, Ident, Token, TokenKind};
use uwuc_base::sym::Symbol;
use uwuc_base::{Context, FileId, Providers, Span};
...
impl<'a> Lexer<'a> {
    ...
    fn number(&mut self) -> Option<TokenKind> {
            ...
            && self.peek2().map(|c| c.is_ascii_digit()).unwrap_or_default()
        {
            ...
            let Ok(num) = f64::from_str(s).map_err(|_| self.error(ErrorKind::InvalidFloat, Span::new(self.start, self.current, self.file))) else { return None };
