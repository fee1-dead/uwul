...
use uwuc_base::ast::TyKind;
use uwuc_base::data::FxHashMap;
use uwuc_base::errors::ErrorReported;
use uwuc_base::hir::{Func, HirTree, ItemFn, Literal, Resolution};
use uwuc_base::mir::{
    BasicBlockData, Body, Function, Local, LocalData, MirTree, Operand, Rvalue, Statement, Targets,
    Terminator,
};
use uwuc_base::{hir, Context, ContextExt, FileId, Id, Providers};

fn add_mir(cx: &dyn Context, HirTree { functions, items }: HirTree, v: &mut Vec<(Id, Function)>) -> Result<(), ErrorReported> {
    v.reserve(items.len());
    ...
    for item in items.iter() {
        match item {
            ...
            hir::Item::Mod { name: _, tree } => {
                add_mir(cx, tree.clone(), v)?;
            }
        }
    }
    Ok(())
...
fn mir(cx: &dyn Context, id: FileId) -> Result<MirTree, ErrorReported> {
    ...
    add_mir(cx, hir, &mut item)?;
    
    let items = Rc::new(item.into_iter().collect());
