...
use uwuc_base::ast::{BinOpKind, TyKind, UnOpKind};
use uwuc_base::data::FxHashMap;
use uwuc_base::errors::ErrorReported;
use uwuc_base::hir::{Literal, Resolution};
use uwuc_base::mir::{self, Function, Local, Operand, Rvalue, Statement, Terminator};
use uwuc_base::sym::{self, Symbol};
use uwuc_base::{Context, FileId, Id, Providers, TyList};
...
impl<'a, 'cx> LlvmCodegen<'a, 'cx> {
    ...
    pub fn basic_ty(&mut self, ty: TyKind) -> BasicTypeEnum<'a> {
        match ty {
            ...
            TyKind::String => self.llcx.i8_type().ptr_type(AddressSpace::default()).into(),
            // x => todo!("{x:?}"),
        }
    }
    ...
    pub fn literal(&mut self, c: &Literal) -> BasicValueEnum<'a> {
        match c {
            ...
            Literal::String(s) => self
                .builder
                .build_global_string_ptr(s.get_str(), "global").unwrap()
                .as_pointer_value()
                .into(),
            x => todo!("{x:?}"),
        }
            ...
            (
                ...
                match binop {
                    $(BinOpKind::$binop => match (a, b) {
                        // TODO error handling
                        $((BasicValueEnum::$value(a), BasicValueEnum::$value(b)) => self.builder.$name($($($args)*,)? a, b, "").unwrap().into(),)*
                        x => todo!("{x:?}"),
                    },)*
                    x => todo!("{x:?}"),
                }
    ...
    pub fn rvalue(&mut self, rv: &Rvalue) -> BasicValueEnum<'a> {
        match rv {
            ...
            Rvalue::UnaryOp(UnOpKind::Minus, a) => match self.operand(a) {
                BasicValueEnum::IntValue(x) => self.builder.build_int_neg(x, "").unwrap().into(),
                _ => todo!(),
            },
            x => todo!("{x:?}"),
        }
        ...
        *self.c_printf.get_or_insert_with(|| {
            self.module.add_function(
                ...
                self.llcx.void_type().fn_type(
                    &[self.llcx.i8_type().ptr_type(AddressSpace::default()).into()],
                    true,
                ),
                None,
            )
        ...
        match (sym, &*types) {
            (sym::println, &[ty]) => {
                ...
                let fmt_global = if let Some(g) = self.module.get_global(&fmt_global) {
                    ...
                    builder.build_global_string_ptr(fmt_value, &fmt_global).unwrap()
                };

                let p = func.get_first_param().unwrap();
                let val = if ty == TyKind::Bool {
                    let ty = self.llcx.i8_type().ptr_type(AddressSpace::default());
                    let s = ty.const_array(&[
                        self.builder
                            .build_global_string_ptr("false", "false_value").unwrap()
                            .as_pointer_value(),
                        self.builder
                            .build_global_string_ptr("true", "true_value").unwrap()
                            .as_pointer_value(),
                    ]);
    ...
    fn local(&mut self, l: Local) -> BasicValueEnum<'a> {
        if l.index() < self.fun.unwrap().count_params() as usize {
            ...
            self.builder.build_load(self.locals[&l], "").unwrap()
        }
    }
    fn gen_function(&mut self, id: Id, f: &Function) -> FunctionValue<'a> {
        ...
        let locals: FxHashMap<_, _> = f
            ...
            .map(|(local, data)| {
                ...
                let ptr = self.builder.build_alloca(ty, &format!("{local:?}")).unwrap();
                (local, ptr)
            })
            .collect();
        self.locals = locals;
